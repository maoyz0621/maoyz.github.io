...
```sh
docker run --rm --name redis -p 6379:6379 -v /opt/docker/redis/redis-one/redis.conf:/etc/redis/redis.conf -v /opt/docker/redis/redis-one/data:/data -d redis:5.0 redis-server /etc/redis/redis.conf --appendonly yes
```
...
# 配置主机
replicaof 192.168.107.128 6379
# 连接主机的密码
masterauth root
requirepass root
```
...
```
docker run --name redis-master -v /data/redis-5/sentinel/redis.conf:/etc/redis/redis.conf -d -p 6379:6379 redis:5.0 redis-server /etc/redis/redis.conf --appendonly yes
```
...
```shell
## 进入docker运行容器内
docker exec -it 06dcfc1456c7 /bin/bash
...
##### master服务器：192.168.107.128

```
192.168.107.128:6379> info replication
# Replication
role:master
connected_slaves:2
slave0:ip=192.168.107.129,port=6379,state=online,offset=1022,lag=1
slave1:ip=192.168.107.130,port=6379,state=online,offset=1022,lag=0
master_replid:69da4a3d3a96573d48608c537c6ac36711ba80f3
...
- role：担当的角色（master、slave）
- connected_slaves：连接的从机
- master_replid：
- master_replid2：
- master_repl_offset：

##### slave1服务器：192.168.107.129

```
192.168.107.129:6379> info replication
# Replication
role:slave
master_host:192.168.107.128
master_port:6379
...
##### slave1服务器：192.168.107.130

```shell
192.168.107.130:6379> info replication
# Replication
role:slave
master_host:192.168.107.128
master_port:6379
...
dir /tmp
sentinel announce-ip 192.168.107.129 # 本机
...
```
docker run --name redis-sentinel --net=host -v /data/redis-5/sentinel/sentinel.conf:/etc/redis/sentinel.conf -d -p 26379:26379 redis:5.0 redis-sentinel /etc/redis/sentinel.conf
```

启动成功之后查看sentinel.conf：
...
dir "/tmp"
sentinel myid 8f909ca4a5049bd85b127417b1cc320b0c2c770d
sentinel deny-scripts-reconfig yes
...
sentinel known-replica mymaster 192.168.107.130 6379
sentinel known-replica mymaster 192.168.107.129 6379
# Generated by CONFIG REWRITE
sentinel known-sentinel mymaster 192.168.107.130 26379 e772457dd0a4f1003787a2114f7f1589a8a90ec2
sentinel known-sentinel mymaster 192.168.107.128 26379 208d0531ec4b95fee47dec5a5561f7cee55b1795
sentinel current-epoch 0
sentinel announce-ip "192.168.107.129"
sentinel announce-port 26379
```
...
master0:name=mymaster,status=ok,address=192.168.107.128:6379,slaves=2,sentinels=3
```

**master0:name=mymaster,status=ok,address=192.168.107.128:6379,slaves=2,sentinels=3**，监控的master=192.168.107.128:6379，从服务器slaves=2，哨兵进程sentinels=3
