...
> Spring是如何解决循环依赖的？
>
> 通过三级缓存解决循环依赖，并且只能解决Setter注入的循环依赖，请大家思考一下如何解决？为何只能是Setter注入？
...
- 请求到 **DispatcherServlet** 前端控制器
- **HandlerMapping** 处理器映射器，解析对应请求的Handler。
- **HandlerAdapter** 处理器适配器，调用真正的处理器处理请求、业务逻辑。
- 返回**ModelAndView**，ViewResolver视图解析器，返回**View**给请求者。