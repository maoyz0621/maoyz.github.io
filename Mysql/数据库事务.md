...
注解@Transactional，当业务代码中抛出异常时，Spring会创建RuleBasedTransactionAttribute，对@Transactional的描述
...
其中TransactionAspectSupport#`getTransactionAttribute()`获取的是TransactionAttribute，其实现类是AnnotationTransactionAttributeSource，那么getTransactionAttributeSource方法主要逻辑是交给了computeTransactionAttribute方法。
...
通过computeTransactionAttribute处理逻辑，可以看出，对于非公共方法，则返回null。如果在接口、实现类或方法上都指定了@Transactional 注解，则优先级顺序为方法>实现类>接口；
